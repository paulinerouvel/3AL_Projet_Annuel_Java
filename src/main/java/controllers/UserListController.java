...
public class UserListController {
    ...
    private void displayUsers() {
        ...
        users = services.User.fetchAllUsers();
    ...
    public void activateUser(ActionEvent event){
        ...
        if(indexOfUserSelected != -1) {
            try {
                ...
                if(services.User.updateUser(userElement) < 299){
                    System.out.println(services.User.notifyByMailUser(userElement));
                }
    ...
    public void refuseProduct(ActionEvent event) {
        ...
        if(indexOfUserSelected != -1) {
            try {
                JSONObject user = users.getJSONObject(indexOfUserSelected);
                User userElement = services.User.jsonToUser(user);
                userElement.setEstValide(0);

                Integer updateResult = services.User.updateUser(userElement);
    ...
    public void displayAddUser(ActionEvent actionEvent) throws Exception {
        ...
        if(indexOfUserSelected != -1) {
            FXMLLoader loader = new FXMLLoader(getClass().getResource("/views/ManageUser.fxml"));
